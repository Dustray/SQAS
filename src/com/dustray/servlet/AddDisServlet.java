...
public class AddDisServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/** 从表单中取得值 **/
		// String brlid=request.getParameter("brlid");//自动递增
		...
		for (int i = 0; i < arr.length; i++) {// 循环添加
			...
			brli.setBrlstuname(arr[i]);
			/** 执行添加操作 **/
			AddDisDao dao = new AddDisDaoImpl();
			dao.addDisciplineInfo(brli);
		}
		response.sendRedirect("InquireServlet?limit=2"); // 页面跳转到查询页面中
	}
